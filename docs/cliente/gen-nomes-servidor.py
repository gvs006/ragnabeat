...
# Preserva um Footer escrito a mao (ex.: Imports apontando para outro .yml).
# Sem isto, regenerar apagaria em silencio um import que alguem adicionou.
rodape = ''
if destino.exists():
    antigo = destino.read_bytes().decode('cp1252', 'replace')
    i = antigo.find('\nFooter:')
    if i >= 0:
        rodape = antigo[i:].rstrip() + '\n'
        print()
        print('Footer preservado (%d linhas):' % len(rodape.strip().split('\n')))
        for l in rodape.strip().split('\n'):
            print('   ', l)

destino.write_bytes(('\n'.join(linhas) + '\n' + rodape).encode('cp1252'))
print()
